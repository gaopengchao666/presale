...
public class Page
{
    ...
    private void refreshPage()
    {
        ...
        topPageSize = (pageNo == totalPage && totalRecord % pageSize != 0)
                ? totalRecord % pageSize : pageSize;
        // sqlserver 不包含的数量
    ...
    private String computeDisplayStyleAndPage()
    {
        ...
        //自己写方法提单google collection
        StringBuffer sb = new StringBuffer();
        for(Integer number : pageDisplays)
        {
            sb.append(number);
            if (pageDisplays.indexOf(number) < pageDisplays.size() - 1)
            {
                sb.append("|");
            }
        }
